...
This app (`twinframe`) is a client-side celebrity face-matching web app (TanStack Start + Vite). The active on-device pipeline is ONNX Runtime Web (`onnxruntime-web`) with an EdgeFace 512-d embedder + SCRFD detector (legacy `@vladmandic/face-api`/tfjs code is still present but no longer the primary path). Standard commands live in `README.md` / `package.json` (`npm run dev|build|typecheck|test|lint`). Notes below are the non-obvious gotchas for this environment.

- ONNX Runtime WASM assets: the four runtime WASM files that actually load (`ort-wasm-simd-threaded.{jsep,asyncify,jspi}.wasm` and `ort-wasm-simd-threaded.wasm`) are committed as real ~13–27 MB binaries, so `npm run dev` works with no extra step. The three unused variants (`ort-wasm-simd.wasm`, `ort-wasm-threaded.wasm`, `ort-wasm.wasm`) remain 24-byte placeholders. `npm run copy:ort` (`node scripts/copy-ort-assets.mjs`, also run inside `npm run build`) re-copies the real assets from `node_modules/onnxruntime-web/dist`; on the current committed state it is a no-op (leaves git clean). If a future checkout ever ships placeholders instead, `copy:ort` restores them — the update script runs it after `npm install` for safety. Do NOT commit regenerated WASM binaries.
- In-browser matching completes in this VM. Uploading a photo → crop review → "Approve & Match" returns real celebrity matches (top match + feature breakdown + alternates) in headless Chrome within a few seconds via the `onnxruntime-web` WASM backend — the older face-api/tfjs "~21s/face past the 20s timeout" behavior no longer applies to the primary path. Browser QA of the full flow is the best end-to-end check.
- Alternative CPU harness (no browser): `node scripts/evaluate-accuracy.mjs --tier 1 --limit 8 --concurrency 4 --verbose` runs the real detect→align→embed→gallery-match pipeline on CPU (~24s/face) and reports Top-1/Top-5 accuracy + per-probe matches (small samples score 100% Top-1). A full 270-probe run takes ~30+ min on CPU, so use `--limit`.
- `npm test` (~330 tests) uses `node --experimental-strip-types` and needs no browser. There is currently 1 pre-existing app-logic failure in `src/lib/face/m4-challenger-stress.test.ts` (`rankByDescriptor` returns 1 result instead of the expected 2) — it is unrelated to environment setup. `npm run lint` (33 errors / 114 warnings) and `npm run typecheck` (errors in `src/routes/re-encode.tsx`, `src/routes/held-out-encode.tsx`) also report pre-existing issues in app code — these are not environment problems.
- Browser test/verify scripts (`scripts/*.mjs`, `tests/`) use Playwright; the update script runs `npx playwright install chromium`. For software rendering in headless runs, launch Chromium with `--use-gl=angle --use-angle=swiftshader --enable-unsafe-swiftshader`. Note: `scripts/browser-smoke.mjs` intercepts routes and serves from a built `.vercel/output`, so it returns 404 against the plain `npm run dev` server — QA the dev server with a normal browser/computer-use session instead.
