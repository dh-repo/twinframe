...
async function embedImage(session, entry, filePath) {
  ...
  let img = sharp(filePath).removeAlpha().resize(size, size, { fit: "cover" });
  ...
  for (let i = 0; i < stride; i++) {
    ...
    const g = data[i * 3 + 1];
    const b = data[i * 3 + 2];
    if (entry.normalizeFrom0to255) {
      float[i] = r / 255;
      float[i + stride] = g / 255;
      float[i + 2 * stride] = b / 255;
    } else {
      float[i] = (r - entry.mean) / entry.std;
      float[i + stride] = (g - entry.mean) / entry.std;
      float[i + 2 * stride] = (b - entry.mean) / entry.std;
    }
  }
  const feeds =
    entry.layout === "nhwc"
      ? { [session.inputNames[0]]: new ort.Tensor("float32", float, [1, size, size, 3]) }
      : { [session.inputNames[0]]: new ort.Tensor("float32", float, [1, 3, size, size]) };
