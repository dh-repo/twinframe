...
| `scripts/evaluate-held-out-v2.ts` **v2.1 leak-excluded, full 512-d geometry** | 301 clean | **79.7% Rank-1**, 80.4% Rank-5, MRR 0.801 | Probes from photos that contribute to **no** gallery artifact (by path and by content hash), encoded through the same SCRFD → align → AdaFace IR-101 512-d path the browser runs. Tracked in `reports/held-out-v2-baseline.json`. CI gates Rank-1 ≥ 75%. |
