...
| `scripts/evaluate-held-out-v2.ts` **v2.1 leak-excluded, full 512-d geometry** | 301 clean | **74.8% Rank-1**, 80.4% Rank-5, MRR 0.771 | Probes from photos that contribute to **no** gallery artifact (by path and by content hash), encoded through the same SCRFD → align → EdgeFace-512d path the browser runs |
...
Treat **74.8% held-out Rank-1** as the honest headline. Composition notes: enrolling 26 previously-missing celebrities made 33 more probe ids evaluable; adding six identity-verified multi-shot templates later shifted individual outcomes both ways and left like-for-like accuracy flat. Protocol details: probes re-encoded
