...
| `scripts/evaluate-held-out-v2.ts` **v2.1 leak-excluded, full 512-d geometry** | 270 clean | **75.9% Rank-1**, 80.7% Rank-5, MRR 0.779 | Probes from photos that contribute to **no** gallery artifact (by path and by content hash), encoded through the same SCRFD → align → EdgeFace-512d path the browser runs |
...
Treat **75.9% held-out Rank-1** as the honest headline. Protocol details: probes re-encoded
